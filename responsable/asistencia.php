...
// Obtener proyecto asignado al responsable (uno solo)
$proyecto = $conn->query("SELECT p.* FROM empleados e JOIN proyectos p ON e.proyecto_id = p.id WHERE e.id = $user_id AND p.activo=1")->fetch_assoc();
if (!$proyecto) { die('No tienes proyecto asignado.'); }
